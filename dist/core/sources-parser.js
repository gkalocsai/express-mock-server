...
class SourcesParser {
  ...
  switchRecord(record) {
    ...
    this.map[path][method].forEach((changedRecord, index) => {
      ...
      if (isQueryStringEqual(queryStringParameters, changedRecord.queryStringParameters) && JSON.stringify(body) === JSON.stringify(changedRecord.body)) {
        ...
        this.map[path][method][index] = new Record(record); // console.log('CHANGE TO '.blue);
        // console.log(JSON.stringify(this.map[path][method][index], null, "  "));
  ...
  sortMap() {
    Object.keys(this.map).forEach(path => {
      ...
      let gets = pathItem[_constants.methods.GET];

      if (gets) {
        gets.sort(compare);
      }

      let posts = pathItem[_constants.methods.POST];

      if (posts) {
        posts.sort(compare);
      }

      let puts = pathItem[_constants.methods.PUT];

      if (puts) {
        puts.sort(compare);
      }

      let deletes = pathItem[_constants.methods.DELETE];

      if (deletes) {
        deletes.sort(compare);
      }

      let patches = pathItem[_constants.methods.PATCH];
...
exports.SourcesParser = SourcesParser;
...
function compare(a, b) {
  ...
  if (b.queryStringParameters.length != a.queryStringParameters.length) return b.queryStringParameters.length - a.queryStringParameters.length; // toto posledni pravidlo je kvuli obecnemu parametru, aby prevazili konkretni nad obecnym.   2 > .*
...
//# sourceMappingURL=sources-parser.js.map